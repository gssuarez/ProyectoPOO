    ...
    private static void venderBoleto(Empleado empleado){
        ...
        while(opcion < planificaciones2.size() && opcion< 0){
            ...
            for(int i = 0; i<planificaciones2.size();i++){ 
                System.out.println(String.valueOf(i+1)+".- Fecha: "+ planificaciones2.get(i).getDepartuteTime().toString() 
           +  " | Asientos primera clase: "+planificaciones2.get(i).getAsientosPrimClase().getDisponibles() + " | Asientos normales: "+planificaciones2.get(i).getAsientosNormClase().getDisponibles());}
        ...
        if (op.equals("1")){
            asiento = reserva.getAsientosPrimClase();  
        }    
        else if (op.equals("2")){
            asiento = reserva.getAsientosNormClase();  
        ...
        System.out.printf("Existen %d asiento disponibles\n",asiento.getDisponibles());
            ...
            while(!op.equals("SI") && !op.equals("NO") && !op.equals("SALIR")){
                System.out.print("Tiene cuenta de cliente(SI / NO): ");
                op = scanner.next().toUpperCase();
            }
            if(op.equals("SI")){
                cliente ="";
                while((Cliente.existeCliente(cliente) || cliente.equals(""))){
                    System.out.print("Ingrese cedula(SALIR SI QUIERE ABORAR OPERACION): ");
                ...
                Sistema.clientes.add(client);
            }
            double total = boletos * asiento.getPrecio() ;
            if(LocalDate.now().getYear() - client.getAñoNacimiento() < 2){
                System.out.println("Usted ha aplicado a un descuento de edad");
                total=total/2;
            }
            System.out.printf("El total es: %5.2f dolares\n",total);
            op="";
            ...
            if (op.equals("SI")){    
                ...
                while(boletos!=0){
                    r.getBoletos().add(new Boleto(client,asiento.generarPuesto(),reserva.getCod_vuelo()));
                    boletos--;
                }
