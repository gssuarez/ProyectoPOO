...
public class Sistema {
    ...
    private static void cargarPlanificaciones(){
        for(String s: Archivo.planificaciones){
            String [] st = s.split(",");
            planificaciones.add(new Planificacion(st[0],st[1],LocalDateTime.parse(st[2]),LocalDateTime.parse(st[3]), st[4],st[5] ));
        }
        for(Planificacion p : planificaciones){
            for(Aerolinea a: aerolineas){
                for(Avion avion: a.getAviones()){
                    if(avion.getNSerie().equals(p.getAvion())){
                        a.getPlanificaciones().add(p);
                    }
                }
            }
        }
    ...
    private static void cargarAsientos(){
         ...
         for (Asiento asiento: asientos){
             Planificacion p = Planificacion.retornarPlanificacion(asiento.getCodVuelo());
             if(asiento.getTipo().equals(Asiento.Tipo.PRIMERA_CLASE)){
                 p.setAsientosPrimClase(asiento);
             }
             else if(asiento.getTipo().equals(Asiento.Tipo.NORMAL_CLASE)){
                 p.setAsientosNormClase(asiento);
             }
         }
    }
    ...
    private static void guardarAvion(){
        ...
        for(Avion avion : aviones){
            String data = avion.getNSerie() + "," + avion.getFabricante() + "," + avion.getModelo() + "," + String.valueOf(avion.getAsientosPrimClase()) + "," 
    ...
    private static void guardarAsiento(){
         ...
         for(Asiento asiento: asientos){
             ...
             { tipo = "NORMAL_CLASE" ;}
             String data = asiento.getCodVuelo() + "," + String.valueOf(asiento.getDisponibles()) + "," + String.valueOf(asiento.getPrecio()) + "," + tipo;
             total.add(data);
            }
         Archivo.guardarArchivo(total, Archivo.direccionAsiento);
        }
    private static void guardarPlanificacion(){
        ArrayList<String> total = new ArrayList<>();
         for(Planificacion p: planificaciones){
             String data= p.getAvion() + "," + p.getCod_vuelo() + "," + p.getBoardingTime().toString() + "," + p.getDepartuteTime().toString() + "," +
                     p.getIATAArribo() + "," + p.getIATASalida();
             total.add(data);
         }
        Archivo.guardarArchivo(total, Archivo.direccionPlanificacion);
    }
    private static void guardarReserva(){
        ArrayList<String> total = new ArrayList<>();
        for( Reserva r: reservas){
            //String cod_vuelo, LocalDate fecha, String user_empleado, double total, int cantidadPasajeros
            String data = r.getCodVuelo() + "," + r.getFecha().toString() + "," + r.getUserEmpleado() + "," + String.valueOf(r.getTotal()) + "," + String.valueOf(r.getCantidadPasajeros());
            total.add(data);
         }
        Archivo.guardarArchivo(total, Archivo.direccionReserva);
    }
}
